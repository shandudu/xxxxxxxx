...
export interface AlertInboxItem {
  source: 'QUALITY' | 'ANDON' | 'INVENTORY' | 'SALES' | 'PURCHASING' | string;
  alert_type: string;
  alert_id: number;
  code: string;
  title: string;
...
export interface AlertInboxSummary {
  total: number;
  open_count: number;
...
export async function getAlertInboxApi(params?: {
  source?: string;
  status?: string;
  limit?: number;
}) {
  return requestClient.get<AlertInboxSummary>('/api/v1/monitors/alerts', {
    params,
  });
}
