...
begin
    ...
    values ('routing.operationMenu', 'MesOperation', '/mes/operation', 13, 'mdi:format-list-numbered', 1, '/plugins/routing/views/operation', 'mes:operation:view', 1, 1, 1, '', null, (select id from sys_menu where name = 'System'), now(), null) returning id into operation_menu_id;
    insert into sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
    values ('routing.workCenterMenu', 'MesWorkCenter', '/mes/work-center', 14, 'mdi:factory', 1, '/plugins/routing/views/work-center', 'mes:workcenter:view', 1, 1, 1, '', null, (select id from sys_menu where name = 'System'), now(), null) returning id into work_center_menu_id;
    insert into sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
    values ('routing.menu', 'MesRouting', '/mes/routing', 15, 'mdi:source-branch', 1, '/plugins/routing/views/routing', 'mes:routing:view', 1, 1, 1, '', null, (select id from sys_menu where name = 'System'), now(), null) returning id into routing_menu_id;
    insert into sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
    values
