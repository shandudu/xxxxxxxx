...
select setval(pg_get_serial_sequence('mes_unit', 'id'), coalesce(max(id), 0) + 1, true) from mes_unit;
select setval(pg_get_serial_sequence('mes_material_category', 'id'), coalesce(max(id), 0) + 1, true) from mes_material_category;
