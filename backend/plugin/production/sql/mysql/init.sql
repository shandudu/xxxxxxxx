...
values ('production.menu', 'MesProduction', '/mes/production', 18, 'mdi:factory', 1, '/plugins/production/views/index', 'mes:production:view', 1, 1, 1, '', null, @system_menu_id, now(), null);
set @production_menu_id = LAST_INSERT_ID();
insert into sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
values
...
('生产报工', 'MesProductionReport', null, 0, null, 2, null, 'mes:production:report', 1, 0, 1, '', null, @production_menu_id, now(), null);
