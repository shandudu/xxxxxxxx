do $$
declare
    supplier_menu_id bigint;
begin
    insert into sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
    values ('supplier.menu', 'ErpSupplier', '/erp/supplier', 21, 'mdi:truck-delivery-outline', 1, '/plugins/supplier/views/index', null, 1, 1, 1, '', null, (select id from sys_menu where name = 'System'), now(), null)
    returning id into supplier_menu_id;
